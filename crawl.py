...
if __name__ == '__main__':
    ...
    # Write a python file with the version number, version.py
    with open('eurovoc/version.py', 'w') as f:
        f.write(f"version = '{version}'\n")
    # Write a json file with the thesaurus_en_labels named labels.py
    with open('eurovoc/thesaurus_en_labels.py', 'w') as f:
        f.write(f"labels = {json.dumps(thesaurus_en_labels(), indent=2)}\n")

    with open('eurovoc/thesaurus_uris.py', 'w') as f:
        f.write(f"identifiers = {json.dumps(thesaurus_uris(), indent=2)}\n")

    with open('eurovoc/en_labels.py', 'w') as f:
        f.write(f"labels = {json.dumps(Eurovoc().labels, indent=2)}\n")

    with open('eurovoc/en_identifiers.py', 'w') as f:
        f.write(f"identifiers = {json.dumps(Eurovoc().identifers, indent=2)}\n")
